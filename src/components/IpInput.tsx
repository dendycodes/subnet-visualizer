  ...
  return (
    <div className="glass flex flex-col gap-6 rounded-3xl p-5 sm:p-7">
      <div className="flex flex-col gap-2">
        ...
        >
          IP address <span className="text-white/25">(or paste as 192.168.1.0/24)</span>
        </label>
        <motion.div
          animate={error ? { x: [0, -8, 8, -6, 6, -2, 2, 0] } : { x: 0 }}
          transition={{ duration: 0.45 }}
          className={`flex items-center gap-3 rounded-2xl border bg-black/20 px-4 py-3.5 transition-colors ${
            error
              ? "border-rose-500/60 shadow-[0_0_0_3px_rgba(244,63,94,0.15)]"
              : "border-white/10 focus-within:border-sky-400/60 focus-within:shadow-[0_0_0_3px_rgba(56,189,248,0.15)]"
          }`}
        >
          <input
            id="ip-input"
            ...
            className="mono w-full flex-1 bg-transparent text-lg font-semibold text-white outline-none placeholder:text-white/20 sm:text-2xl"
          />
          <span className="mono text-lg font-semibold text-white/30 sm:text-2xl">
            /{prefix}
          </span>
        </motion.div>
        {error && (
      ...
      <div className="flex flex-col gap-3">
        ...
        <input
          ...
          className="slider h-2 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-sky-400"
        />
