  ...
  return (
    <div className="glass flex flex-col gap-6 rounded-3xl p-5 sm:p-7">
      <div className="flex flex-col gap-2">
        <label
          htmlFor="ip-input"
          className="text-xs font-medium uppercase tracking-wider text-ink/65 dark:text-ink/45"
        >
          IP address <span className="text-ink/45 dark:text-ink/25">(or paste as 192.168.1.0/24)</span>
        </label>
        <motion.div
          ...
          className={`flex items-center gap-3 rounded-2xl border bg-(--well) px-4 py-3.5 transition-colors ${
            error
              ...
              : "border-ink/20 dark:border-ink/10 focus-within:border-sky-400/60 focus-within:shadow-[0_0_0_3px_rgba(56,189,248,0.15)]"
          }`}
        >
          <input
            id="ip-input"
            ...
            className="mono w-full flex-1 bg-transparent text-lg font-semibold text-ink outline-none placeholder:text-ink/40 dark:text-ink/20 sm:text-2xl"
          />
          <span className="mono text-lg font-semibold text-ink/50 dark:text-ink/30 sm:text-2xl">
            /{prefix}
          </span>
        </motion.div>
        {error && (
            ...
            <button
              ...
              className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
                prefix === p
                  ...
                  : "border-ink/20 dark:border-ink/10 bg-ink/[0.045] dark:bg-ink/[0.02] text-ink/65 dark:text-ink/50 hover:border-ink/38 dark:hover:border-ink/25 hover:text-ink/90 dark:hover:text-ink/80"
              }`}
            >
              /{p}
            </button>
